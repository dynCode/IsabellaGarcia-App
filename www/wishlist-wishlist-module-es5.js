...
var WishlistPage = /** @class */ (function () {
    function WishlistPage(productsService, authenticationService, storage, pageDetail) {
    ...
    WishlistPage.prototype.ngOnInit = function () {
        ...
        this.storage.ready().then(function (data) {
            _this.storage.get("wishlist").then(function (data) {
                ...
                console.log("Wishlist Items", _this.listItems);
                if (_this.listItems.length > 0 || _this.listItems.length !== null) {
                    _this.listItems.forEach(function (item, index) {
                        _this.total = _this.total + (item.product.price * item.qty);
                    });
                }
                else {
                    _this.showEmptyListMessage = true;
                }
            });
        });
    };
    WishlistPage.prototype.replaceIMG = function (imgString) {
        var newstr = imgString.replace("beta.", "");
        return '<img src="' + newstr + '">';
    };
    WishlistPage.prototype.removeFromList = function (item, i) {
        var _this = this;
        var price = item.product.price;
        var qty = item.qty;
        this.listItems.splice(i, 1);
        this.storage.set("wishlist", this.listItems).then(function () {
    ...
    WishlistPage.prototype.getProdDet = function (id) {
        this.productsService.ProductDetails(id);
        console.log(this.productsService.productDetails);
    };
    WishlistPage.prototype.searchByKeyword = function () {
